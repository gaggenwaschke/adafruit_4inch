...
void GUI_DrawCircle(POINT X_Center, POINT Y_Center, LENGTH Radius, COLOR Color,
  ...
  if (Draw_Fill == DRAW_FULL) {
    ...
    while (XCurrent <= YCurrent) {
      GUI_DrawPoint(X_Center + XCurrent, Y_Center + YCurrent, Color, Dot_Pixel,
                    DOT_STYLE_DFT); // 1
      GUI_DrawPoint(X_Center - XCurrent, Y_Center + YCurrent, Color, Dot_Pixel,
                    DOT_STYLE_DFT); // 2
      GUI_DrawPoint(X_Center - YCurrent, Y_Center + XCurrent, Color, Dot_Pixel,
                    DOT_STYLE_DFT); // 3
      GUI_DrawPoint(X_Center - YCurrent, Y_Center - XCurrent, Color, Dot_Pixel,
                    DOT_STYLE_DFT); // 4
      GUI_DrawPoint(X_Center - XCurrent, Y_Center - YCurrent, Color, Dot_Pixel,
    ...
    for (Column = 0; Column < Font->Width; Column++) {
      ...
      if (FONT_BACKGROUND ==
        ...
        if (pgm_read_byte(ptr) & (0x80 >> (Column % 8)))
          GUI_DrawPoint(Xpoint + Column, Ypoint + Page, Color_Foreground,
                        DOT_PIXEL_DFT, DOT_STYLE_DFT);
      } else {
        if (pgm_read_byte(ptr) & (0x80 >> (Column % 8))) {
          GUI_DrawPoint(Xpoint + Column, Ypoint + Page, Color_Foreground,
          ...
          GUI_DrawPoint(Xpoint + Column, Ypoint + Page, Color_Background,
                        DOT_PIXEL_DFT, DOT_STYLE_DFT);
        }
      }
      // One pixel is 8 bits
...
void GUI_DisGrayMap(POINT Xpoint, POINT Ypoint, const unsigned char *pBmp) {
  ...
  if (Gray == 0x04) { // Sixteen gray levels
    ...
    for (j = 0; j < Height; j++)
      for (i = 0; i < Width / 2; i++) {
        GUI_DrawPoint(Xpoint + i * 2, Ypoint + j, ~(*pBmp >> 4), DOT_PIXEL_DFT,
                      DOT_STYLE_DFT);
        GUI_DrawPoint(Xpoint + i * 2 + 1, Ypoint + j, ~*pBmp, DOT_PIXEL_DFT,
                      DOT_STYLE_DFT);
        pBmp++;
      }
  } else {
...
void GUI_Showtime(POINT Xstart, POINT Ystart, POINT Xend, POINT Yend,
    ...
    if ((pTime->Sec / 10) < 6 && (pTime->Sec / 10) > 0) {
      ...
      if ((pTime->Min % 10) < 10 && (pTime->Min % 10) > 0) {
        ...
        if ((pTime->Min / 10) < 6 && (pTime->Min / 10) > 0) {
          LCD_SetArealColor(Xstart + Dx * 2 + Dx / 2, Ystart, Xend, Yend,
                            WHITE); // xx:00:00
        } else {                    // min = 60
  ...
  GUI_DisChar(Xstart + Dx + Dx / 4 + Dx / 2, Ystart, ':', Font, FONT_BACKGROUND,
              Color);
...
void GUI_Show(void) {
  ...
  if (sLCD_DIS.LCD_Dis_Column >
    ...
    GUI_DrawRectangle(10, 30, sLCD_DIS.LCD_Dis_Column - 10,
                      ...
                      DOT_PIXEL_DFT);
    GUI_DrawRectangle(20, 40, sLCD_DIS.LCD_Dis_Column - 20, 60, BLUE, DRAW_FULL,
                      DOT_PIXEL_DFT);
    ...
    GUI_DisString_EN(80, 80, "WaveShare Electronic", &Font24, LCD_BACKGROUND,
                     BLUE);
    ...
    GUI_DisString_EN(40, 120, "WaveShare Electronic", &Font24, LCD_BACKGROUND,
                     BLUE);
